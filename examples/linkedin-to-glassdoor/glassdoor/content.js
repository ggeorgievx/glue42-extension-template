  ...
  (async () => {
    ...
    const setStatusToConnected = () => {
      ...
      if (typeof injectedGlue !== 'undefined') {
        tooltipElement.textContent = `Extension: Glue42 Extension Template Examples LinkedIn to GlassDoor (GlassDoor); Version: ${injectedGlue.version}; Username: ${injectedGlue.connection.resolvedIdentity.user}; GW URL: ${injectedGlue.config.connection.ws};`;
      }
    };
...
// Register a method named 'T42.GlassDoor.SyncCompany' that changes the location.href to the search results for the company provided inside of args
injectedGlue.agm.register('T42.GlassDoor.SyncCompany', ({ company }) => {
  location.href = `https://www.glassdoor.com/Reviews/${company.replace(/\s/g, '-')}-SRCH_KE0,${company.length}.htm`;
});
